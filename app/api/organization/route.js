...
export async function POST(req){
   ...
   // await connectDB();
    const {orgName,
           ...
           orgType,
           password,
           confirmPassword} = await req.json();
    ...
    if(!orgName || !phone || !email || !address || !city || !state || !country 
        || !password || !confirmPassword){
            return NextResponse.json({message: "Some required fields are missing"}, {status:400})
        }
    if(!isValidEmail){
        return NextResponse.json({message: "Email is not valied"}, {status: 400})
    }
    ...
    try{
        ...
        */
        ...
        if(existingOrganization || existingOrganizationPhone || existingOrganization){
            return NextResponse.json({message: "Organization with given phone or email or Organization Name exists!"}, {status:400})
        }
        ...
        if(createdOrg){
            ...
            if(newOrgUserCretaed){
                return NextResponse.json({message: "Organization registered succussfully!"}, {status: 201})
            }else{
                await createdOrg.delete();
                return NextResponse.json({message: "Failed to Register Organization"}, {status: 400})
            }
