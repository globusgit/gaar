...
export async function POST(req){
    try{
        ...
        const receivable = await ReceivableInfo.create(body);
        console.log("Created Receivable Info: " + receivable);
        
        return NextResponse.json({message: "Receivable Info created successfully!"}, {status:200})
    }catch(err){
        return NextResponse.json({message: "Something went wrong!"},{status:500})
    } 
}
...
export async function PUT(req) {
    ...
    try {
      ...
      const updated = await ReceivableInfo.findByIdAndUpdate(
        id,
        rest,
        { new: true }
      )

      return NextResponse.json({message: "Receivable Info updated successfully!"}, {status:200})
    } catch (err) {
      return NextResponse.json({message: "Something went wrong!"},{status:500})
    }
}
